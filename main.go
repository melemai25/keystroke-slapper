...
import (
	...
	"github.com/gopxl/beep/v2/speaker"*/ //for sounds
...
func run(ctx context.Context, tuning runtimeTuning) error {
	...
	return listenForSlaps(ctx, accelRing, tuning)
}
...
if stdioMode {
	...
	fmt.Printf("slap #%d [%s amp=%.5fg] -> ENTER KEY\n", num, ev.Severity, ev.Amplitude)
}
go sendEnterKey()
	}
}*/

func listenForSlaps(ctx context.Context, accelRing *shm.RingBuffer, tuning runtimeTuning) error {
	det := detector.New()
	var lastAccelTotal uint64
	var lastEventTime time.Time
	var lastYell time.Time
	...
	fmt.Printf("spank: listening for slaps in keystroke mode with %s tuning... (ctrl+c to quit)\n", presetLabel)
	...
	for {
		...
		lastYell = now
		fmt.Printf("slap detected [%s amp=%.5fg] -> ENTER KEY\n", ev.Severity, ev.Amplitude)
		go sendEnterKey()
	}
}
